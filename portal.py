...
# 3. توزيع الطوابق (نفس الترتيب في ملف الإدارة)
FLOORS = {
...
if not df_hab.empty:
    ...
    if choice != "-- اختر شقتك من القائمة --":
        # استخراج رقم الشقة من النص المختارة
        try:
            ...
            if not res_mask.empty:
                ...
                # جلب القيم (الحالة والدين اليدوي)
                ...
                if not is_resident:
                    ...
                    # الأولوية للدين الذي عدلته أنت يدوياً في الجدول
                    if not pd.isna(dette_manual):
                        final_dette = float(dette_manual)
                    else:
                        # حساب آلي احتياطي (شهر الحالي * 1000 - المدفوع)
    ...
    st.warning("جاري جلب البيانات من السحابة... تأكد من اتصال الإنترنت.")
