...
import calendar
import json # أضفنا هذا السطر لتحويل نص الـ JSON

# إعداد الصفحة
st.set_page_config(page_title="إقامة 28 - Bloc B", page_icon="🏢")

# --- الاتصال بـ Firebase (عن طريق Secrets) ---
if 'db' not in st.session_state:
    try:
        # قراءة المفتاح من الـ Secrets بدلاً من الملف
        key_dict = json.loads(st.secrets["firebase_key"])
        st.session_state.db = firestore.Client.from_service_account_info(key_dict)
    except Exception as e:
        st.error("❌ فشل الاتصال بقاعدة البيانات. تأكد من إعداد Secrets بشكل صحيح.")
        st.stop()
...
def load_portal_data():
    try:
        ...
        if not df_hab.empty and "Appart" in df_hab.columns:
            df_hab["Appart"] = pd.to_numeric(df_hab["Appart"], errors="coerce").astype("Int64")
        
        # إضافة خانة "غير مقيم" (Absent) في التحقق
        if not df_hab.empty and "is_absent" not in df_hab.columns:
...
if not df_hab.empty:
    ...
    if choice != "-- اختر رقم الشقة --":
        ...
        if not resident_info.empty:
            ...
            st.info(f"👤 الساكن: **{row['Nom']}**")
            
            # التحقق من حالة "غير مقيم"
            ...
            # منطق الحساب الجديد (إذا كان غائباً لا يحسب عليه دين)
            if is_absent:
                st.warning("✅ وضعية خاصة: غير مقيم (معفى من الاشتراكات)")
                dette = 0
            else:
